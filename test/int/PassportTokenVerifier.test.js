...
describe('PassportTokenVerifierTest tests', () => {
  ...
  it('should fail because of an invalid signature', async () => {
    ...
    const jwtStrategy = createJwtStrategy(tokenPropertyExtractor);
    passport.use(jwtStrategy);

    function failCallback (extra, success, challenge, status) {
      expect(extra).toBeNull();
      expect(success).toBe(false);
      expect(challenge.name).toBe('JsonWebTokenError');
      expect(challenge.message).toBe('invalid signature');
  ...
  it('should fail because expired', async () => {
    ...
    function failCallback (extra, success, challenge, status) {
      expect(extra).toBeNull();
      expect(success).toBe(false);
      expect(challenge.name).toBe('TokenExpiredError');
      expect(challenge.message).toBe('jwt expired');
      expect(status).toBeUndefined();
  ...
  it.skip('confirm a valid token', async () => {
    ...
    function callCallback (extra, successOrUser, challengeOrInfo, status) {
      expect(extra).toBeNull();
      expect(successOrUser.name).toBe('cobar@carbon.super');
      expect(challengeOrInfo.roles).toBeDefined();
      expect(status).toBeUndefined();
