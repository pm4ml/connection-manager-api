...
describe('DfspInboundService', async function () {
  ...
  describe('DfspInboundService flow', async function () {
    ...
    it('should create an enrollment from a CSR', async () => {
      const enrollmentResult = await DfspInboundService.createDFSPInboundEnrollment(envId, dfspId, { clientCSR: csr });
      assert.property(enrollmentResult, 'id');
      assert.isNotNull(enrollmentResult.id);
      const enrollmentId = enrollmentResult.id;

      const retrievedEnrollment = await DfspInboundService.getDFSPInboundEnrollment(envId, dfspId, enrollmentId);
      ...
      const signResponse = await DfspInboundService.signDFSPInboundEnrollment(envId, dfspId, enrollmentId);
      assert.equal(signResponse.state, 'CERT_SIGNED');

      const certifiedEnrollment = await DfspInboundService.getDFSPInboundEnrollment(envId, dfspId, enrollmentId);
