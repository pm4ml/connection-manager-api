...
const TABLE_NAME = 'dfsp_server_certs';

module.exports = class DfspServerCertsModel extends BaseCrudModel {
  constructor () {
    super(TABLE_NAME);
  }
  ...
  async findByEnvIdDfspId (envId, dfspId) {
    ...
    if (rows.length === 0) {
      throw new NotFoundError(`${this.baseTable} with ids: envId ${envId} dfspId ${dfspId}`);
